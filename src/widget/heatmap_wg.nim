...
const forbiddenKeyBind = {
  Key.Tab, Key.Escape, Key.None, Key.Up, Key.Down, Key.Left, Key.Right, Key.PageUp,
...
proc newHeatmap*(
  ...
  return newHeatmap(
    px, py, width, height, id, title, data, rowLabels, colLabels, colorScheme, border,
    statusbar, enableHelp, bgColor, fgColor, cellWidth, cellHeight, tb,
  )
...
proc calculateMaxVisible*(hm: Heatmap) =
  let availableWidth =
    hm.x2 - hm.x1 - (if hm.showLabels: 10 else: 0) -
    (if hm.showColorbar: hm.colorbarWidth else: 0)
  ...
  hm.showScrollIndicators =
    (hm.scrollOffsetX + hm.maxVisibleCols < totalCols) or
    (hm.scrollOffsetY + hm.maxVisibleRows < totalRows) or hm.scrollOffsetX > 0 or
    hm.scrollOffsetY > 0
...
proc renderHeatmapCells(hm: Heatmap) =
  ...
  let endCol = min(
    hm.scrollOffsetX + hm.maxVisibleCols,
    if hm.data.len > 0:
      hm.data[0].len
    else:
      0,
  ...
  for row in hm.scrollOffsetY ..< endRow:
    for col in hm.scrollOffsetX ..< endCol:
      if row < hm.data.len and col < hm.data[row].len:
            ...
            if x < hm.x2 - (if hm.showColorbar: hm.colorbarWidth else: 0) and y < hm.y2:
              if hm.showValues and h == 0 and w == 0:
                ...
                if valueStr.len <= hm.cellWidth:
                  hm.tb.write(
                    x, y, fg, bg, valueStr[0 ..< min(valueStr.len, hm.cellWidth)]
                  )
                else:
        ...
        if hm.showGrid:
          # Right border
          if cellX + hm.cellWidth <
              hm.x2 - (if hm.showColorbar: hm.colorbarWidth else: 0):
            for h in 0 ..< hm.cellHeight:
...
proc renderLabels(hm: Heatmap) =
  ...
  for col in hm.scrollOffsetX ..< endCol:
    ...
    if labelX < hm.x2 - (if hm.showColorbar: hm.colorbarWidth else: 0):
      let label =
        if col < hm.colLabels.len:
          hm.colLabels[col]
        else:
          $col
      let truncatedLabel =
        if label.len > hm.cellWidth:
          label[0 ..< hm.cellWidth]
        else:
          label
      hm.tb.write(labelX, hm.y1, hm.fg, hm.bg, truncatedLabel)

proc renderColorbar(hm: Heatmap) =
  if not hm.showColorbar:
    return
  ...
  for i in 0 ..< colorbarHeight:
    let value =
      hm.minValue + (hm.maxValue - hm.minValue) * (1.0 - i.float / colorbarHeight.float)
    let (fg, bg) = hm.getColorForValue(value)
...
proc renderScrollIndicators(hm: Heatmap) =
  if not hm.showScrollIndicators:
    return
...
proc cycleColorScheme(hm: Heatmap, args: varargs[string]) =
  case hm.colorScheme
  of RedScale:
    hm.colorScheme = BlueScale
  of BlueScale:
    hm.colorScheme = GreenScale
  of GreenScale:
    hm.colorScheme = GrayScale
  of GrayScale:
    hm.colorScheme = Rainbow
  of Rainbow:
    hm.colorScheme = RedScale
  ...
  if hm.events.hasKey("statusbar"):
    ...
    let totalCols =
      if hm.data.len > 0:
        ...
        0
    let scrollInfo =
      " | View: " & $(hm.scrollOffsetY + 1) & "-" &
      $(min(hm.scrollOffsetY + hm.maxVisibleRows, hm.data.len)) & " x " &
      $(hm.scrollOffsetX + 1) & "-" &
      $(min(hm.scrollOffsetX + hm.maxVisibleCols, totalCols))

    hm.statusbarText =
      " " & schemeStr & " | Size: " & $hm.data.len & "x" & $totalCols & scrollInfo & " "
    ...
    let indicators =
      if hm.showGrid and hm.showLabels and hm.showColorbar and hm.showValues:
        "[G][L][B][V]"
      elif hm.showGrid and hm.showLabels and hm.showColorbar:
        "[G][L][B]"
      elif hm.showGrid and hm.showLabels and hm.showValues:
        "[G][L][V]"
      elif hm.showLabels and hm.showColorbar and hm.showValues:
        "[L][B][V]"
      elif hm.showGrid and hm.showLabels:
        "[G][L]"
      elif hm.showGrid and hm.showColorbar:
        "[G][B]"
      elif hm.showLabels and hm.showColorbar:
        "[L][B]"
      elif hm.showGrid:
        "[G]"
      elif hm.showLabels:
        "[L]"
      elif hm.showColorbar:
        "[B]"
      elif hm.showValues:
        "[V]"
      else:
...
method wg*(hm: Heatmap): ref BaseWidget =
  hm
...
proc createRandomHeatmapData*(
    rows, cols: int, minVal = 0.0, maxVal = 100.0
): HeatmapData =
