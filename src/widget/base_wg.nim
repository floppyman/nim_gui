...
method call*(
    this: ref BaseWidget, event: string, args: varargs[string]
): void {.base.} =
  echo ""
...
method onError*(this: ref BaseWidget, errorCode: string) {.base.} =
  ...
  this.tb.write(
    ...
    fgRed,
    bgWhite,
    "[!] " & wrapWords(errorCode, this.width - this.posX),
    resetStyle,
  )
...
proc toConsoleWidth*(w: float): int =
  (consoleWidth().toFloat * w).toInt

proc toConsoleHeight*(h: float): int =
  (consoleHeight().toFloat * h).toInt
...
proc fill*(
  ...
  if x1 < tb.width and y1 < tb.height:
    let
      c = TerminalChar(ch: ch.runeAt(0), fg: fgColor, bg: bgColor, style: tb.getStyle)
...
proc contains*(wg: ref BaseWidget, x, y: int): bool =
  ...
  result =
    x >= wg.posX and x < wg.posX + wg.width and y >= wg.posY and y < wg.posY + wg.height
...
proc onMouse*(
    wg: ref BaseWidget,
    handler: proc(wg: ref BaseWidget, mouseInfo: MouseInfo) {.closure.},
) =
