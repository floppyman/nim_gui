...
const allowCtrlKeys = {
  ...
  Key.CtrlR, Key.CtrlS, Key.CtrlT, Key.CtrlU, Key.CtrlW, Key.CtrlX, Key.CtrlV,
  Key.CtrlY, Key.CtrlZ,
}
...
proc on*(
  ib: InputBox, key: Key, fn: EventFn[InputBox]
...
proc newInputBox*(
  ...
  result.on(
    Key.CtrlC,
    proc(ib: InputBox, args: varargs[string]) =
      if ib.value.len > 0:
...
proc newInputBox*(
  ...
  return newInputBox(
    px, py, width, height, title, val, modeChar, border, statusbar, bgColor, fgColor, tb
  )
...
proc renderStatusbar(ib: InputBox) =
  if ib.events.hasKey("statusbar"):
    ...
    ib.tb.write(
      ib.x2 - cursorStr.len, ib.height, bgBlue, fgWhite, cursorStr, resetStyle
    )

method render*(ib: InputBox) =
  if not ib.illwillInit:
    return
  ib.clear()
  ib.renderBorder()
  ib.renderTitle()
  if ib.cursor < ib.value.len:
    ib.tb.write(
      ...
      ib.mode,
      resetStyle,
      ib.visualVal.substr(0, ib.visualCursor - 1),
      styleBlink,
      styleUnderscore,
      ib.style.bgColor,
...
proc cursorMove(ib: InputBox, direction: CursorDirection) =
  ...
  of Left:
    if ib.cursor >= 1:
      ...
      ib.cursor = 0
    let (s, e, vcursorPos) =
      rtlRange(ib.value, (ib.width - ib.posX - ib.paddingX1 - 1), ib.cursor)
  ...
  of Right:
    ...
    let (s, e, vcursorPos) =
      ltrRange(ib.value, (ib.width - ib.posX - ib.paddingX1 - 1), ib.cursor)
...
method onUpdate*(ib: InputBox, key: Key) =
  ...
  if ib.value.len >= ib.width - ib.paddingX1 - 1:
    ...
    let (s, e, cursorPos) =
      rtlRange(ib.value, (ib.width - ib.posX - ib.paddingX2 - 1), ib.cursor)
    #let (s, e, cursorPos) = rtlRange(ib.value, (ib.width - ib.paddingX1 - 1), ib.cursor)
    ib.visualVal = ib.value.substr(s, e)
    ib.visualCursor = cursorPos
  else:
    let (s, e, cursorPos) =
      ltrRange(ib.value, (ib.width - ib.posX - ib.paddingX2 - 1), ib.cursor)
