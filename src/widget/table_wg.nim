import illwill, sequtils, base_wg, options, strutils, parsecsv, input_box_wg, display_wg
...
const forbiddenKeyBind =
  {Key.Tab, Key.Escape, Key.Slash, Key.Up, Key.Down, Key.Left, Key.Right}
...
proc newTable*(
    ...
    maxColWidth = w,
    mouseEnabled: bool = false,
    tb = newTerminalBuffer(w + 2, h + py + 4),
): Table =
  ...
  if headers.isSome:
    table.size -= 1
    let seqCol = newTableColumn(
      seqColWidth, 1, text = alignLeft("i", seqColWidth), key = "", index = 0
    )
...
proc newTable*(
  ...
  return newTable(
    px, py, width, height, rows, headers, id, title, border, statusbar, enableHelp,
    bgColor, fgColor, selectionStyle, maxColWidth, mouseEnabled, tb,
  )
...
proc emptyRows(
    table: Table, emptyMessage = "No records", bgColor = bgRed, fgColor = fgWhite
) =
  if table.events.hasKey("empty"):
    table.call("empty", "")
  else:
    table.tb.write(
      table.posX + table.paddingX1,
      table.posY + 3,
      bgColor,
      fgColor,
      center(emptyMessage, table.width - table.paddingX1 - 2),
      resetStyle,
    )
...
proc renderClearRow(table: Table, index: int, full = false) =
  if full:
    ...
    table.tb.fill(table.posX, table.posY, totalWidth, table.height, " ")
  else:
    table.tb.fill(
      table.posX + table.paddingX1,
      table.posY + index,
      table.width - table.posX + table.paddingX1,
      table.posY + index,
      " ",
    )
    ...
    for i in table.colCursor ..< table.headers.get.columns.len:
      if table.headers.get.columns[i].visible and posX < table.width:
        ...
        table.tb.write(
          ...
          styleBright,
          if i == table.colCursor: styleUnderscore else: styleBright,
          alignLeft(
            table.headers.get.columns[i].text,
            min(
              table.headers.get.columns[i].width,
    ...
    of sdDown:
      let rowSize =
        if table.mode == Filter:
          ...
          table.rows.len
      table.rowCursor = min(table.rowCursor + 3, max(rowSize - table.size, 0))
        # Scroll down 3 lines
    else:
      discard
...
proc renderTableRow(table: Table, row: TableRow, index: int) =
  ...
  for i in 0 ..< row.columns.len:
    ...
    if row.visible and table.headers.get.columns[i].visible and posX < table.width:
      if row.selected and
          (table.selectionStyle == Arrow or table.selectionStyle == HighlightArrow):
      ...
      if row.selected and
          (table.selectionStyle == Highlight or table.selectionStyle == HighlightArrow):
        bgSelected = bgGreen
        table.tb.write(
          table.posX + posX,
          table.posY + index,
...
method resize*(table: Table) =
  let statusbarSize = 2
  let padding = 1
  table.size = table.height - table.posY - padding - padding - statusbarSize
  table.maxColWidth = table.x2 - table.x1

method render*(table: Table): void =
  if not table.illwillInit:
    return
...
proc onFilter(table: Table) =
  ...
  var input = newInputBox(
    table.x1, table.y1, table.x2, table.y1 + 2, title = "search", tb = table.tb
  )
...
method onUpdate*(table: Table, key: Key) =
  ...
  of Key.PageDown:
    let size =
      if table.rows.len - table.cursor > table.size:
        table.size - 1
      else:
        table.rows.len - table.cursor - 1
    let rowSize =
      if table.mode == Filter:
        table.vrows().len
      else:
...
proc addRow*(table: Table, tablerow: TableRow, index: Option[int] = none(int)): void =
  for i in 0 ..< tablerow.columns.len:
    if tablerow.columns[i].text.len >= table.headers.get.columns[i].width:
      table.headers.get.columns[i].width =
        min(table.maxColWidth, tablerow.columns[i].text.len + 1)
    if tablerow.columns[i].text.len > table.maxColWidth:
      tablerow.columns[i].text =
        tablerow.columns[i].text[0 .. table.maxColWidth - 3] & ".."
...
proc loadFromCsv*(
    table: Table, filepath: string, withHeader = false, withIndex = false
) =
  try:
    ...
    while readRow(csvparser):
      ...
      if rindex == 0:
        var headerWidth = 0
        if not withIndex:
          var column = newTableColumn(
            ($rindex).len + 1, 1, "s/q", $rindex, $rindex, rindex, columnType = Header
          )
          row.add(column)
        for val in items(csvparser.row):
          var column = newTableColumn(
            val.len + 1, 1, val, $rindex, val, rindex, columnType = Header
          )
          row.add(column)
        ...
        if not withIndex:
          var column = newTableColumn(
            ($rindex).len, 1, $rindex, $rindex, $rindex, rindex, columnType = Column
          )
          row.add(column)
        for val in items(csvparser.row):
          var col =
            newTableColumn(val.len, 1, val, $rindex, val, rindex, columnType = Column)
          row.add(col)
          rowWidth += val.len
...
proc headerFromArray*(
    table: Table, header: openArray[string], bgColor = bgNone, fgColor = fgWhite
) =
  if header.len == 0:
  ...
  for h in header:
    let column = newTableColumn(h.len, 1, h, h, bgColor = bgColor, fgColor = fgColor)
